...
public class MyReactor implements Closeable, Runnable {
	private final ServerSocketChannel serverChannel;
	private final Selector selector;
	public MyReactor() throws IOException{
	...
	public void run() {
		try {
			...
			serverChannel.register(selector, SelectionKey.OP_ACCEPT, new MyAccepter(serverChannel, selector));
			while (true) {
	...
	public static void main(String[] args) {
		//创建Reactor对象，启动，完事
		try (MyReactor reactor = new MyReactor()){
			reactor.run();
		}catch (IOException e) {
			e.printStackTrace();
		}
